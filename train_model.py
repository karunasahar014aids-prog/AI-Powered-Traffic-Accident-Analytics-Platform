...
DATA_CANDIDATES = [DATA_DIR / "dataset.csv", DATA_DIR / "victorian_road_crash_data(1).csv"]
OFFICIAL_URL = "https://opendata.transport.vic.gov.au/dataset/bb77800e-1857-4edc-bf9e-e188437a1c8e/resource/5df1f373-0c90-48f5-80e1-7b2a35507134/download/victorian_road_crash_data.csv"
...
def find_dataset():
    for path in DATA_CANDIDATES:
        if path.exists():
            return path
    target = DATA_DIR / "dataset.csv"
    print("Downloading official Victorian Road Crash Data...")
    urllib.request.urlretrieve(OFFICIAL_URL, target)
    return target
...
def load_data():
    ...
    required = ["SPEED_ZONE", "ACCIDENT_TYPE", "LIGHT_CONDITION", "ROAD_GEOMETRY", "DAY_OF_WEEK", "NO_OF_VEHICLES", "SEVERITY"]
    ...
    return df[FEATURES + [TARGET]].dropna(subset=[TARGET]).copy()
...
def main():
    ...
    classifier = RandomForestClassifier(n_estimators=150, min_samples_leaf=2, class_weight="balanced", random_state=42, n_jobs=-1)
    ...
    metrics = {
        ...
        "classes": sorted(y.unique().tolist()), "rows": int(len(df)), "dataset": "Victorian Road Crash Data",
